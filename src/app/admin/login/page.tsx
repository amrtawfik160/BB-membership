    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          {error && (
            <div className={`rounded-md p-4 ${rateLimited ? 'bg-red-50 border border-red-200' : 'bg-yellow-50 border border-yellow-200'}`}>
              <div className="flex">
                <AlertCircle className={`h-5 w-5 ${rateLimited ? 'text-red-400' : 'text-yellow-400'}`} />
                <div className="ml-3">
                  <p className={`text-sm ${rateLimited ? 'text-red-800' : 'text-yellow-800'}`}>
                    {error}
                  </p>
                </div>
              </div>
